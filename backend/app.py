...
def api_locations(token: str = Depends(require_token)):
    ...
    b, t = hb.base(), hb.timeout()
    h = {"Authorization": f"Bearer {token}"}
    if version == "v2":
        r = requests.get(f"{b}/api/v1/entities?isLocation=true&pageSize=100", headers=h, timeout=t, verify=False)
    else:
        r = requests.get(f"{b}/api/v1/locations?pageSize=100", headers=h, timeout=t, verify=False)
